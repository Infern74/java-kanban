...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Task hometask = new Task("Cделать дз", "По истории и информатике");
        taskManager.addTask(hometask);

        Task readBook = new Task("Прочитать книгу", "Java epam");
        taskManager.addTask(readBook);

        Task newReadBook = new Task("Прочитать книгу", "Java epam", readBook.getId(),TaskStatus.IN_PROGRESS);
        taskManager.updateTask(newReadBook);

        Epic project = new Epic("Проект","2 семестр");
        taskManager.addEpic(project);
        Subtask projectSubtask1 = new Subtask("План", "Составить этапы",
                project.getId());
        ...
        Subtask cleaningSubtask1 = new Subtask("Помыть полы", "В каждой комнате", cleaning.getId());
